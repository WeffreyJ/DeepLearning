...
class Trainer:
    def __init__(self, model, optimizer, loss_fn, device: Optional[str] = None, ckpt_dir: str = "saved_models",
        ...
        self.ckpt_dir = ckpt_dir; os.makedirs(self.ckpt_dir, exist_ok=True)
    ...
    def train_one_epoch(self, loader: DataLoader):
        ...
        for x, y in tqdm(loader, desc="Train", leave=False):
            ...
            b = y.size(0)
            running_loss += loss.item() * b; running_acc += acc * b; n += b
    ...
    def evaluate(self, loader: DataLoader):
        ...
        for x, y in tqdm(loader, desc="Eval", leave=False):
            ...
            loss = self.loss_fn(logits, y)
            b = y.size(0)
            running_loss += loss.item() * b; running_acc += accuracy(logits, y) * b; n += b
        return running_loss/n, running_acc/n

    def fit(self, epochs: int, train_loader: DataLoader, val_loader: DataLoader, ckpt_name: str = "model.pth"):
        best_acc = 0.0; ckpt_path = os.path.join(self.ckpt_dir, ckpt_name)
        for epoch in range(1, epochs+1):
            ...
            if self.csv_logger: self.csv_logger.log({"epoch": epoch, "train_loss": tr_loss, "train_acc": tr_acc, "val_loss": va_loss, "val_acc": va_acc})
            if va_acc > best_acc:
                best_acc = va_acc; torch.save({"model_state": self.model.state_dict()}, ckpt_path)
