...
def build_scheduler(cfg, optimizer):
    ...
    if name == "cosine":
        return optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=int(scfg.get("t_max", 10)))
    return None
...
def main():
    ...
    if cfg.get("task", "") == "vision_classification":
        ...
        loss_fn = nn.CrossEntropyLoss(label_smoothing=label_smoothing if label_smoothing > 0 else 0.0)
        out_dir = cfg.get("output", {}).get("dir", "saved_models"); os.makedirs(out_dir, exist_ok=True)
        csv_log = cfg.get("output", {}).get("csv_log", None)
        patience = cfg.get("training", {}).get("early_stopping_patience", None)
        mixup_alpha = float(cfg.get("training", {}).get("mixup_alpha", 0.0))
        trainer = Trainer(model, optimizer, loss_fn,
                          device=cfg.get("training", {}).get("device", "auto"),
                          ckpt_dir=out_dir, scheduler=scheduler, csv_log_path=csv_log,
                          early_stopping_patience=patience, mixup_alpha=mixup_alpha)
        best = trainer.fit(int(cfg["training"]["epochs"]), train_loader, val_loader, ckpt_name="best.pth")
        print(f"Best val acc: {best:.4f}")
    else:
        raise SystemExit("This starter wires up CIFAR10 & MNIST vision training. Extend for NLP or other tasks.")
